...
sudo git clone https://github.com/prashantsakharkar/certs.git /etc/kolla/certificates/
sudo git clone https://github.com/prashantsakharkar/keyrings.git /etc/kolla/config/
sudo sed -e '/kolla_internal_vip_address/ s/^#*/#/' -i /etc/kolla/globals.yml
#sudo sed -i 's/1048576//g' /usr/local/share/kolla-ansible/ansible/roles/neutron/tasks/precheck.yml
sudo sed -i '/host_key_checking = False/s/^#//g' /etc/ansible/ansible.cfg
sudo sed -i '/#pipelining = False/c pipelining = True' /etc/ansible/ansible.cfg

echo "kolla_base_distro: "centos"
kolla_install_type: "binary"
openstack_release: "train"
kolla_internal_vip_address: "10.10.10.240"
kolla_external_vip_address: "172.172.3.240"
kolla_external_fqdn: "kollaautotrain.triliodata.demo"
...
sudo sed  -i '/rbd_secret_uuid/c rbd_secret_uuid = '$secret /etc/kolla/config/cinder/cinder-volume.conf
...
#control02
#control03
172.172.3.239 ansible_ssh_user=root ansible_become=True ansible_ssh_pass=password
...
[compute]
#compute01
172.172.3.238 ansible_ssh_user=root ansible_become=True ansible_ssh_pass=password
[monitoring]
#monitoring01
